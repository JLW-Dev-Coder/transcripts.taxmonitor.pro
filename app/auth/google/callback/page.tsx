...
function GoogleCallbackInner() {
  ...
  useEffect(() => {
    ...
    async function handleCallback() {
      try {
        // Exchange code via VLP worker — it handles the token exchange
        // and returns a handoff token for cross-domain session
        const res = await fetch(
          `${API}/v1/auth/google/callback?code=${encodeURIComponent(code!)}&state=${encodeURIComponent(state || '')}&ttmp=1`,
          { credentials: 'include' }
        )
        ...
        // Check if worker returns a handoff token (cross-domain)
        const contentType = res.headers.get('content-type') || ''
        if (contentType.includes('application/json')) {
          const data = await res.json()
          if (data?.handoffToken) {
            sessionStorage.setItem('ttmp_session_id', data.handoffToken)
        ...
        // If worker redirected, follow it
        window.location.href = '/app/dashboard/'
      } catch (err: unknown) {
